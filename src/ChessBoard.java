public class ChessBoard {
    ...
    public boolean moveToPosition(ChessBoard chessBoard, int startLine, int startColumn, int endLine, int endColumn) {
        if (checkPos(startLine) && checkPos(startColumn)) {
            if (!nowPlayer.equals(chessBoard.board[startLine][startColumn].getColor())) return false;
            if (chessBoard.board[startLine][startColumn].canMoveToPosition(this, startLine, startColumn, endLine, endColumn)) {
                if(chessBoard.board[endLine][endColumn] != null && chessBoard.board[endLine][endColumn].getColor().equals(nowPlayer)){
                    return false; //Нельзя есть свои фигуры
                }
                if(chessBoard.board[startLine][startColumn] instanceof King && ((King) chessBoard.board[startLine][startColumn]).isUnderAttack(chessBoard, endLine, endColumn)) {
                    System.out.println("Bad move, your king is under attack");
                    return false; //Проверка на шах, если пытаемся сделать ход королем
                }
                chessBoard.board[endLine][endColumn] = chessBoard.board[startLine][startColumn]; // if piece can move, we moved a piece
                chessBoard.board[startLine][startColumn] = null; // set null to previous cell
                for(int i = 0; i < 8; i ++){
                    for (int z = 0; z < 8; z++){ //Проверка поля перед сохранением результата, если нам поставили шах, то нужно защитить короля
                        if(chessBoard.board[i][z] instanceof King && chessBoard.board[i][z].getColor().equals(nowPlayer) &&
                                ((King)chessBoard.board[i][z]).isUnderAttack(chessBoard, i, z)){
                            System.out.println("You need to defend your king");
                            chessBoard.board[startLine][startColumn] = chessBoard.board[endLine][endColumn];
                            chessBoard.board[endLine][endColumn] = null;
                            return false;
                        }
                    }
                }
                if(chessBoard.board[endLine][endColumn].check){
                    chessBoard.board[endLine][endColumn].check = false;
                }
                if(chessBoard.board[endLine][endColumn].getSymbol().equals("P")){
                    if((chessBoard.board[endLine][endColumn].getColor().equals("White") && endLine == 7) ||
                            (chessBoard.board[endLine][endColumn].getColor().equals("Black") && endLine == 0))//Проверяем не пешка ли делала ход и не достигла ли она конца доски
                         {
                    chessBoard.board[endLine][endColumn] = new Pawn(nowPlayer).changePawn(this, endLine, endColumn); //Превращаем пешку в другую фигуру
                }
                }
